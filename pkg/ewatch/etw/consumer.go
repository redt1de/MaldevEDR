...
func (c *Consumer) callback(er *EventRecord) (rc uintptr) {
	...
	if h, err := newEventRecordHelper(er); err == nil {
		...
		///////////////////////////////////// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		if er.ExtendedDataCount > 0 && er.ExtendedData.ExtType == EVENT_HEADER_EXT_TYPE_STACK_TRACE64 {
			length := int(er.ExtendedData.DataSize) / 8
			s := (*[1 << 30]uint64)(unsafe.Pointer(er.ExtendedData.DataPtr))[:length:length]
			// msg := "StackTrace:"
			tmp := []string{}
			for _, i := range s {
				tmp = append(tmp, fmt.Sprintf("0x%x", i))
			}
			// msg += strings.Join(tmp, ",")
			// event.ExtendedData = append(event.ExtendedData, msg)
			event.EventData["StackTrace"] = tmp
		}
		////////////////////////////////////////>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
